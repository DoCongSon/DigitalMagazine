...
public class CmsController : Controller
{
    ...
    public CmsController(IApi api, IModelLoader loader, IAnalyticsService analyticsService)
    {
        _api = api;
        _loader = loader;
    ...
    public async Task<IActionResult> Post(Guid id, bool draft = false)
    {
        try
        {
            ...
            if (!draft)
            {
                await _analyticsService.LogViewAsync(model.Id, HttpContext.Connection.RemoteIpAddress?.ToString() ?? "unknown");
            }
